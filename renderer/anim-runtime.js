...
function drawElbowJoinCollars(ctx) {
  const paintCollar = (path, bounds) => {
    ...
    ctx.fill();
    pencilShade(ctx, path, bounds, { wash: 0.048, hatch: 0.026, cross: 8e-3, spacing: 7, texture: 0.018 });
    path();
...
function drawBlockCounter(ctx, part, state) {
  ...
  if (part.counterLabel) {
    ...
    drawCastShadow(ctx, frame2, 3, 4, 0.05);
    frame2();
...
function drawLampBulbHead(ctx, part) {
  ...
  ctx.fillStyle = glass;
  ctx.fill();
  pencilShade(ctx, bulb, { x: -112, y: -122, w: 224, h: 250 }, { wash: 0.045, hatch: 0.018, cross: 6e-3, spacing: 10, texture: 0.018 });
...
function drawSpacemanHelmetHead(ctx, part) {
  ...
  ctx.fill();
  pencilShade(ctx, ring, { x: -116, y: -126, w: 232, h: 252 }, { wash: 0.045, hatch: 0.018, cross: 6e-3, spacing: 10, texture: 0.02 });
  ctx.strokeStyle = "rgba(18,22,25,0.58)";
  ctx.lineWidth = 2.4;
  ring();
  ctx.stroke();
...
function drawFastenerShape(ctx, x, y, type, scale = 1, seed = 1) {
  ...
  if (type === "rivet") {
    ...
    drawPencilRect(ctx, -11, -3.5, 22, 7, 3.5, { wash: 0.075, hatch: 0.03, cross: 8e-3, spacing: 4, shadow: 0.022, lineWidth: 1.1 });
...
function drawPart(ctx, part, state) {
  const def = getPart(part.key);
  if (!def) return;
  const bounds = partBounds({ ...part, scaleX: 1, scaleY: 1 });
  const pad = 96;
...
function applyMouseLean(ctx, width, height, mouseLook, look) {
  ...
  ctx.rotate(x * 0.018);
  ctx.transform(1, -y * 6e-3, -x * 5e-3, 1, 0, 0);
...
function drawMachine(ctx, layout, chainState = {}, options = {}) {
  ...
  const machineLean = mouseLook.active && !useDragStaticCache && options.previewMotion !== false && options.mouseLean !== false && (!editMode || options.mouseLeanInEdit === true);
...
function archiveAgeSeconds(chainState = {}) {
  return secondsFromState(chainState, "archiveAgeSeconds", "ageDays") || secondsFromState(chainState, "ageSeconds", "age");
}
function holderBondSeconds(chainState = {}) {
  return secondsFromState(chainState, "holderBondSeconds", "holderBondDays") || secondsFromState(chainState, "bondSeconds", "bondDays");
}
...
function drawPfpSpecialBackdrop(ctx, width, height, layout, time) {
  ...
  ctx.fillStyle = aura;
  ctx.fillRect(0, 0, width, height);
  ctx.fillStyle = colorAlpha2("#ffffff", 0.08 * intensity);
...
function drawMaterialSkinBackdropMotifs(ctx, width, height, seed, skin, intensity, pulse) {
  ...
  if (["void", "cosmic"].includes(surface)) {
    ctx.save();
    ctx.globalCompositeOperation = "lighter";
    ctx.fillStyle = colorAlpha2("#ffffff", 0.22 * intensity);
    ...
    ctx.save();
    ctx.globalCompositeOperation = "lighter";
    ctx.strokeStyle = colorAlpha2(edge, 0.055 + pulse * 0.025);
    ctx.lineWidth = 0.85;
    for (let i = 0; i < 8; i += 1) {
    ...
    ctx.fillStyle = colorAlpha2("#fff2a6", 0.14 * intensity);
    ...
    ctx.save();
    ctx.globalCompositeOperation = "lighter";
    ctx.strokeStyle = colorAlpha2(accent, 0.12 + pulse * 0.08);
    ctx.lineWidth = 1.4;
    ...
    ctx.save();
    ctx.strokeStyle = colorAlpha2(surface === "blueprint" ? accent : primary, 0.06);
    ctx.lineWidth = 1;
...
function drawSpecialMaterialAtmosphere(ctx, width, height, layout, chainState, time, look, skin) {
  ...
  if (surface === "royalgold") {
    ctx.globalCompositeOperation = "lighter";
    ctx.fillStyle = colorAlpha2("#fff2a6", 0.16 + pulse * 0.05);
  ...
  if (["liquid", "chrome", "resin", "organic", "sludge", "molten", "blackMatter"].includes(surface) || ["blackMatter", "mercuryChrome", "moltenGold"].includes(skin.id)) {
    ctx.globalCompositeOperation = "lighter";
    ctx.fillStyle = colorAlpha2(edge, 0.12 + pulse * 0.08);
...
function buildLiquidState(layout, chainState, time, previewMotion) {
  ...
  const lastChanged = Number(chainState.liquidLastChangedAt || Date.now() / 1e3);
...
function drawPipeLink(ctx, a, b, time, previewMotion, active, liquidState = {}, look = CANVAS_LOOKS.whiteBlueprint) {
  ...
  ctx.lineTo(b.x, b.y);
  ctx.stroke();
  ctx.strokeStyle = active ? colorAlpha2(theme.base, 0.56) : look.guideWeak;
  ctx.lineWidth = 2;
  ctx.setLineDash([8, 9]);
  ctx.lineDashOffset = previewMotion ? -time * 42 : 0;
  ctx.beginPath();
  ...
  ctx.setLineDash([]);
  ctx.lineDashOffset = 0;
  drawFlowDot(ctx, a.x, a.y, b.x, b.y, time, active ? colorAlpha2(theme.accent, 0.84) : look.guideWeak);
  ctx.restore();
}
...
function drawAgePatina(ctx, width, height, layout, seconds, time, look) {
  ...
  ctx.fillStyle = colorAlpha2(level > 0.6 ? "#8a642d" : "#5a4d35", look.pfp ? 0.025 + level * 0.035 : 0.035 + level * 0.05);
...
function drawHolderBondGearMilestones(ctx, width, height, placements, seconds, time, look) {
  ...
  for (let i = 0; i < gears.length; i += 1) {
    ...
    ctx.fill();
    ctx.strokeStyle = colorAlpha2(color, ringAlpha);
    ...
    if (milestone.rust > 0.12) {
      ctx.globalCompositeOperation = "source-over";
      ctx.strokeStyle = colorAlpha2("#8b5a24", look.pfp ? 0.045 + milestone.rust * 0.08 : 0.12 + milestone.rust * 0.18);
...
function drawSaleProvenanceSeal(ctx, width, height, placements, rank, time) {
  ...
  ctx.save();
  ctx.globalCompositeOperation = "lighter";
  ctx.fillStyle = colorAlpha2(highSale ? "#ffef7a" : colors[rank], 0.075 + rank * 0.025 + pulse * 0.018);
  ctx.beginPath();
  ...
  if (highSale) {
    ...
    ctx.shadowColor = colorAlpha2("#ffef7a", 0.5 + pulse * 0.2);
    ...
    ctx.strokeStyle = colorAlpha2("#ffef7a", 0.94);
    ctx.lineWidth = 2.4;
    diamond();
    ctx.fill();
    ctx.stroke();
    ...
    ctx.restore();
    ctx.strokeStyle = colorAlpha2("#d7a13a", 0.7 + pulse * 0.18);
    ctx.lineWidth = 1.25;
    for (let i = 0; i < 12; i += 1) {
    ...
    ctx.fillStyle = colorAlpha2("#fff8d7", 0.96);
    ...
    ctx.shadowColor = colorAlpha2("#d7a13a", 0.5 + pulse * 0.2);
    ctx.shadowBlur = 9;
    gear();
...
function saleEventCount(chainState = {}) {
  const explicit = Number(
    chainState.saleCount ?? chainState.sellCount ?? chainState.sales ?? chainState.saleTransfers ?? chainState.soldTransfers ?? chainState.verifiedSaleCount ?? 0
  );
...
function drawBodyPlateSaleScarScreen(ctx, width, height, placements, chainState, look = CANVAS_LOOKS.whiteBlueprint) {
  ...
  plate();
  ctx.stroke();
  ctx.strokeStyle = colorAlpha2(palette.frame, 0.82);
  ctx.lineWidth = 1.35;
  screen();
  ctx.stroke();
  ctx.save();
  screen();
  ctx.clip();
  ctx.fillStyle = colorAlpha2(palette.accent, 0.04 + heat * 0.02);
  ...
  ctx.shadowColor = colorAlpha2(palette.accent, 0.7);
  ctx.shadowBlur = 4;
  drawPixelSaleCount(ctx, sales, 0, y + 12, 3.05, colorAlpha2(palette.accent, 0.95));
  ctx.shadowBlur = 0;
  ctx.restore();
  ctx.save();
  ctx.fillStyle = colorAlpha2(palette.accent, 0.84);
  ctx.font = "bold 4.8px ui-monospace, SFMono-Regular, Menlo, monospace";
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.fillText("SALE", -8, y + 38);
  ctx.fillStyle = sales > 9 ? colorAlpha2(palette.frame, 0.86) : "rgba(176,42,37,0.82)";
  ctx.beginPath();
  ctx.arc(x + plateW - 11, y + 38, 2.1, 0, Math.PI * 2);
  ctx.fill();
  ctx.restore();
  for (const [rx, ry] of [[x + 6, y + 8], [x + plateW - 6, y + 8], [x + 7, y + plateH - 7], [x + plateW - 7, y + plateH - 7]]) {
    ctx.fillStyle = colorAlpha2(palette.frame, 0.88);
    ctx.beginPath();
...
function drawTransferCut(ctx, x1, y1, x2, y2, seed, color, width = 2.1, glow = 0) {
  ...
  if (glow) {
    ...
    ctx.strokeStyle = colorAlpha2(color, 0.18 + glow * 0.22);
    ctx.lineWidth = width + 4;
    drawHistoryLine(ctx, x1, y1, x2, y2, seed + 60, 0.55);
    ctx.globalCompositeOperation = "source-over";
    ctx.shadowBlur = 0;
  }
  ctx.strokeStyle = colorAlpha2(color, 0.92);
  ctx.lineWidth = width;
...
function drawTransferSeal(ctx, x, y, label, seed, options = {}) {
  ...
  ctx.fill();
  ctx.stroke();
  ctx.strokeStyle = glow ? colorAlpha2("#66f5dd", 0.18 + glow * 0.24) : "rgba(215,161,58,0.5)";
...
function drawScreenPulse(ctx, bounds, heartbeat, pressure, time, color) {
  ...
  ctx.strokeStyle = colorAlpha2(color, alpha);
  ...
  for (let i = 0; i < lineCount; i += 1) {
    ...
    ctx.beginPath();
    ctx.moveTo(bounds.x + bounds.w * 0.2 + jitter2, y);
    ctx.lineTo(bounds.x + bounds.w * 0.82 + jitter2 * 0.4, y + Math.sin(i + time) * 2);
    ctx.stroke();
  }
  ctx.restore();
}
function drawFluidReaction(ctx, bounds, heartbeat, pressure, time, color) {
  ...
  ctx.save();
  ctx.strokeStyle = colorAlpha2(color, 0.2 + heartbeat * 0.18);
  ctx.fillStyle = colorAlpha2(color, 0.08 + boil * 0.05);
  for (let i = 0; i < 12; i += 1) {
...
function drawElectricReaction(ctx, bounds, heartbeat, pressure, time, color) {
  ...
  if (!active) return;
  ctx.save();
  ctx.strokeStyle = colorAlpha2(pressure === "extreme" ? "#ffef7a" : color, 0.18 + heartbeat * 0.3);
...
function drawArchiveReaction(ctx, bounds, heartbeat, pressure, time, color) {
  ctx.save();
  ctx.strokeStyle = colorAlpha2(color, 0.16 + heartbeat * 0.22);
  ctx.fillStyle = colorAlpha2("#fff4c8", 0.05 + heartbeat * 0.05);
  for (let i = 0; i < 5; i += 1) {
...
function drawVaultReaction(ctx, bounds, heartbeat, pressure, time, color) {
  ctx.save();
  ctx.strokeStyle = colorAlpha2(color, 0.16 + heartbeat * 0.25);
  ctx.lineWidth = 2;
  ...
  ctx.stroke();
  if (pressure === "high" || pressure === "extreme") {
    ctx.strokeStyle = colorAlpha2("#ffb347", 0.16);
