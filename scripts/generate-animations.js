...
const layoutDir = path.join(root, "build", "layouts");
const animationDir = path.join(root, "build", "animations");
const publicAnimationDir = path.join(root, "web", "public", "living-archive", "animations");
...
function copyRuntimeModules(targetDir) {
  for (const fileName of ["renderer.js", "parts.js", "schema.js", "liveChain.js"]) {
    fs.copyFileSync(path.join(root, "web", "src", fileName), path.join(targetDir, fileName));
  }
}

function numericLayoutFiles() {
  if (!fs.existsSync(layoutDir)) {
    throw new Error(`Missing ${layoutDir}. Run npm run lam:layouts first.`);
  }
...
<style>
  * { box-sizing: border-box; }
  ...
  .controls button {
    ...
    height: 20px;
    padding: 0;
    border: 1px solid rgba(235,211,121,.48);
    background:
      linear-gradient(135deg, rgba(255,242,160,.96), rgba(49,89,82,.78) 58%, rgba(8,20,20,.86));
    color: rgba(5,13,13,.96);
    font: 900 9px/1 ui-monospace, Menlo, Consolas, monospace;
    cursor: pointer;
    text-shadow: 0 1px 0 rgba(255,255,255,.28);
    box-shadow:
      1px 1px 0 rgba(16,38,37,.36),
      0 0 10px rgba(236,178,53,.18),
      inset 0 1px 0 rgba(255,255,255,.35);
  }
  .controls button:hover,
  .controls button.is-active {
    border-color: rgba(122,245,220,.82);
    background:
      linear-gradient(135deg, rgba(127,255,229,.98), rgba(18,128,117,.9) 62%, rgba(5,18,20,.92));
    color: rgba(3,16,16,.98);
    box-shadow:
      1px 1px 0 rgba(16,38,37,.28),
      0 0 14px rgba(122,245,220,.28),
      inset 0 1px 0 rgba(255,255,255,.4);
  }
</style>
</head>
<body>
<main class="stage" aria-label="${layout.name} assembly viewer">
  <canvas id="render" width="${layout.canvas.width}" height="${layout.canvas.height}"></canvas>
...
<script type="module" src="./${layout.tokenId}.js"></script>
</body>
</html>
`;
}

function scriptFor(layout) {
  return `import { drawMachine } from "./renderer.js";
import { getPartRadius, partBounds } from "./parts.js";

const BASE_LAYOUT = ${escapeScriptJson(layout)};
...
copyRuntimeModules(animationDir);
copyRuntimeModules(publicAnimationDir);

const start = Math.max(1, Number(process.env.LAM_ANIMATION_START || 1));
const count = Number(process.env.LAM_ANIMATION_COUNT || 0);
const layoutFiles = numericLayoutFiles()
  .filter((fileName) => Number(path.basename(fileName, ".json")) >= start)
  .slice(0, count > 0 ? count : undefined);

for (const fileName of layoutFiles) {
  const file = path.join(layoutDir, fileName);
  const layout = JSON.parse(fs.readFileSync(file, "utf8"));
  const tokenId = layout.tokenId;
  const html = htmlFor(layout);
  const js = scriptFor(layout);
  fs.writeFileSync(path.join(animationDir, `${tokenId}.html`), html);
  fs.writeFileSync(path.join(animationDir, `${tokenId}.js`), js);
  fs.writeFileSync(path.join(publicAnimationDir, `${tokenId}.html`), html);
  fs.writeFileSync(path.join(publicAnimationDir, `${tokenId}.js`), js);
  console.log(`animation Motorhead #${tokenId}: OpenSea assembly viewer`);
}

console.log(`Wrote ${layoutFiles.length} OpenSea assembly animation pages.`);
