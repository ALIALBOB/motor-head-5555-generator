/* Render the 3 effect ERC-1155 item cards (Neon / Holo / Teal) on a display machine → PNGs + a montage. */
...
const PORT = 5199, URL = `http://127.0.0.1:${PORT}/effect-card.html`;
const TOKEN = Number(process.env.TOKEN || 1);
const OUTDIR = process.env.OUTDIR || path.join(os.homedir(), "Downloads", "motorheads-effect-cards");
const chrome = process.env.CHROME_PATH || "C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe";
// [filename, fx key, floor-glow accent, label]
const JOBS = [
  ["1-neon", "neon", "#39f6ff", "NEON GLOW"],
  ["2-holo", "holo", "#b06bff", "HOLO"],
  ["3-teal", "teal", "#2fd9c4", "TEAL CHROME"],
];
...
(async () => {
  ...
  const port = 9261, prof = path.join(os.tmpdir(), "mh-cards-" + Date.now());
  ...
  try {
    ...
    for (let i = 0; i < 200; i++) { const b = await s("Runtime.evaluate", { expression: "typeof window.renderEffectCard==='function'", returnByValue: true }).catch(() => null); if (b?.result?.value === true) break; await sleep(100); }
    for (const [name, fx, col] of JOBS) {
      const r = await s("Runtime.evaluate", { expression: `window.renderEffectCard(${TOKEN}, ${JSON.stringify(fx)}, ${JSON.stringify(col)})`, awaitPromise: true, returnByValue: true });
      if (r?.result?.value?.ok !== true) { console.log("  !", name, "FAILED", JSON.stringify(r?.result?.value || r?.result)); continue; }
      await sleep(180);
    ...
    if (sharp) {
      const T = 460, L = 34, G = 12, W = 3 * T + 4 * G, H = T + L + 2 * G, comps = [];
      for (let i = 0; i < JOBS.length; i++) {
        const [nm, , , lb] = JOBS[i], x = G + i * (T + G), y = G;
        const f = path.join(OUTDIR, `${nm}.png`);
        if (!fs.existsSync(f)) continue;
        comps.push({ input: await sharp(f).resize(T, T).toBuffer(), left: x, top: y });
        comps.push({ input: Buffer.from(`<svg width="${T}" height="${L}"><rect width="100%" height="100%" fill="#12151b"/><text x="14" y="23" font-family="monospace" font-size="16" fill="#7af5dc" font-weight="bold">${lb}</text></svg>`), left: x, top: y + T });
      }
      await sharp({ create: { width: W, height: H, channels: 3, background: "#0d0f13" } }).composite(comps).png().toFile(path.join(OUTDIR, "_montage.png"));
